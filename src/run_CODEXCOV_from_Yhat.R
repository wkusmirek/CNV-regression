docker run --rm -v /home/wkusmire/CNV-regression:/home/wkusmire/CNV-regression -w /home/wkusmire/CNV-regression/src biodatageeks/cnv-opt-codexcov Rscript -e "library('CODEXCOV');source('/home/wkusmire/CNV-regression/src/run_CODEXCOV_from_Yhat.R');run_CODEXCOV(1, 3, 200, '/home/wkusmire/CNV-regression/data/1_cov_NA.csv', '/home/wkusmire/CNV-regression/data/1_cov_NA_norm_2.csv', '/home/wkusmire/CNV-regression/data/20130108.exome.targets.chr1.bed', '/home/wkusmire/CNV-regression/data/reference_sample_set_kmeans_1.csv', 'calls.csv')"
...
run_CODEXCOV <- function(K_from,
                         ...
                         lmax,
                         input_cov_table,
                         input_cov_table_norm,
                         input_bed,
                         reference_sample_set_file,
                         output_calls_file){

  Y <- read.csv(input_cov_table)
  Yhat <- read.csv(input_cov_table_norm)
  targets <- read.delim(input_bed)
  rownames(Y) <- 1:nrow(Y)
  rownames(targets) <- 1:nrow(targets)

  ref <- IRanges(start = targets[,"st_bp"], end = targets[,"ed_bp"])
  samples <- c('NA06984', 'NA06985') # colnames(Y)
  Y <- Y[,samples]
  Yhat <- Yhat[,samples]

  finalcall <- segment_without_K(Y, Yhat, samples, ref, targets[1,'chr'], lmax, mode = "integer")
